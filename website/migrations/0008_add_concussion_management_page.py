...
def add_concussion_management_page(apps, schema_editor):
    ...
    item = next(
        nav_item for nav_item in content.TREATMENTS_COL2
        if nav_item['slug'] == 'concussion-management'
    )
    data = all_treatment_content()['concussion-management']
    sort_order = next(
        index for index, nav_item in enumerate(content.TREATMENTS_COL2)
        if nav_item['slug'] == 'concussion-management'
    )
...
def remove_concussion_management_page(apps, schema_editor):
    ContentPage = apps.get_model('website', 'ContentPage')
    ContentPage.objects.filter(section='treatments', slug='concussion-management').delete()


class Migration(migrations.Migration):

    dependencies = [
        ('website', '0007_remove_telehealth_page'),
    ]

    operations = [
        migrations.RunPython(add_concussion_management_page, remove_concussion_management_page),
    ]
